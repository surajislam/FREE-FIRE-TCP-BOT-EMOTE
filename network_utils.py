...
async def DeCode_PackEt(input_text):
    try:
        ...
        print(f"[PACKET] Error decoding packet data: {e}")
        return None
...
async def Emote_k(TarGeT, idT, K, V):
    ...
    return await GeneRaTePk((await CrEaTe_ProTo(fields)).hex(), "0515", K, V)
    # return await GeneRaTePk((await CrEaTe_ProTo(fields)).hex(), "0500", K, V)
...
async def GeneRaTePk(Pk, N, K, V):
    ...
    if len(_) == 2:
        ...
        print("[PACKET] Error: Failed to generate packet header - invalid length")
